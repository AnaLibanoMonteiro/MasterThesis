...
def get_out_file_name(intermediates_coverage_file_path):
    ...
    if 'coverage_filtered' in intermediates_coverage_file_path:
        return intermediates_coverage_file_path.replace('coverage_filtered.tsv', new_suffix)
    elif 'coverage' in intermediates_coverage_file_path:
...
parser = argparse.ArgumentParser(description="""
        Takes as input 3 files: (1) normalization factors file, (2) splicing intermediates coverage and
        (3) surrounding coverage, created with 'bedtools coverage'; and the sample group name.
        Returns the events on the file (2) that have coverage significantly higher than
        surrounding nucleotides in file (3).""")
