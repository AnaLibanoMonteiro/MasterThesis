...
## NEW: !!!!
def get_intron_length(line):
    return abs(int(line[2]) - int(line[1]))
...
def get_all_introns_and_length(file_content):
    introns = set()
    introns_and_length = set()
    for line in file_content:
        intron_name = get_intron_name(line)
        introns.add(intron_name)
        introns_and_length.add((intron_name, get_intron_length(line)))
    return introns, introns_and_length
...
def get_splicing_ratio(file_path, out_file_path):
    ...
    """
    header = ['intron_id', 'splicing_ratio', 'numb_spliced_reads', 'numb_unspliced_reads', 'numb_total_reads', 'length']
    ##############################################################################
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ## TODO: Add intron length to header and get it from input file (file_content)
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ##############################################################################

    file_content = read_intersect_file(file_path)
    introns, introns_and_length = get_all_introns_and_length(file_content)
    ...
    for i in introns_and_length:
        ...
        splicing_ratio_per_intron.append([intron, sr, len(spliced), len(unspliced_123 + unspliced_23), \
                                          total_spliced_unspliced, length]) ## Added Length
