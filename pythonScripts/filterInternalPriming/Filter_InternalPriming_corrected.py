...
filename=str(files).split("/")[-1].rstrip(".bam")
coord_strat=False

# Extracring .bam file header
system("samtools view -H {} > {}_header.sam".format(files, filename))
header=str(filename)+"_header.sam"

if strategy == "paired": #Get read 2 in paired-end datasets
	coord_strat=True
	system("samtools view "+files+" | awk '($2 ~ /147|163/)' > "+filename+"_tempR2.sam")
...
system("bamToBed -i {}  > {}.bed".format(files, filename))
...
system("samtools view "+files+" | fgrep -f "+filename+"_woIPri_ID.txt > "+filename+".sam")
...
system("rm -f "+header)
system("rm -f "+filename+".bam") # this line was commented out in this corrected version (and not on the previous)
